...
const AdminPanel: React.FC<AdminPanelProps> = ({ onLogout }) => {
  ...
  const handlePrint = () => {
    try {
      // Ensure focus is on window for the print dialog to attach correctly
      window.focus();
      // Small timeout allows any pending render updates to complete
      setTimeout(() => {
        window.print();
      }, 50);
    } catch (e) {
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white pb-20">
      ...
      {/* We do NOT use 'hidden' class here, but control visibility with a print-only class logic in CSS to avoid Tailwind conflicts */}
      ...
      {/* SCREEN VIEW (Hidden via custom class during print) */}
                 ...
                 <div className="grid grid-cols-2 gap-2">
                    ...
                    >
                       ...
                       {availableManualSlots.length > 0 ? (
                         availableManualSlots.map(t => <option key={t} value={t}>{t}</option>)
                       ) : (
